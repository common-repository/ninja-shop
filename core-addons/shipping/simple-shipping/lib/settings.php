...
function it_exchange_simple_shipping_settings_callback() {
	...
	<div class="wrap">
		...
		<?php
		$url = add_query_arg( array( 'page' => 'it-exchange-settings', 'tab' => 'shipping' ), admin_url( 'admin.php' ) );
		?><p><?php printf( __( 'Settings are located in the %sShipping tab%s on the Ninja Shop Settings page.', 'it-l10n-ithemes-exchange' ), '<a href="' . esc_attr( $url ) . '">', '</a>' ); ?></p>
	</div>
	<?php
}
