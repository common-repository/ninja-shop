...
add_filter( 'ninja_shop_super_widget_valid_states', 'it_exchange_customer_order_notes_add_sw_state' );
...
add_filter( 'ninja_shop_get_super-widget-checkout_after-cart-items_loops', 'it_exchange_add_customer_order_note_to_sw_template_totals_loops', 5 );
...
add_action( 'ninja_shop_processing_super_widget_ajax_customer-order-note', 'it_exchange_customer_order_notes_sw_save_note' );
...
function it_exchange_customer_order_notes_checkout_save_note() {
	...
	if ( ! empty( $_POST['it-exchange-edit-customer-order-note'] ) ) {
		$note = isset( $_POST['it-exchange-customer-order-note'] ) ? sanitize_text_field( $_POST['it-exchange-customer-order-note'] ) : '';
...
add_filter( 'ninja_shop_generate_transaction_object', 'it_exchange_customer_order_notes_txn_object', 10, 2 );
...
add_action( 'ninja_shop_empty_cart', 'it_exchange_customer_order_notes_empty_cart' );
...
add_action( 'ninja_shop_add_transaction_success', 'it_exchange_customer_order_notes_create_activity' );
...
add_action( 'ninja_shop_email_notifications_register_tags', 'it_exchange_customer_order_notes_register_tag' );
