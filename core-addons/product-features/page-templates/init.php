...
class IT_Exchange_Product_Feature_Product_Page_Template {
	...
	*/
	function __construct() {
		if ( is_admin() ) {
			...
			add_filter( 'ninja_shop_fetch_template_override_default_page_template', array( $this, 'replace_template' ) );
			add_filter( 'ninja_shop_fetch_template_override_located_template', array( $this, 'replace_template' ) );
		}
		add_action( 'ninja_shop_enabled_addons_loaded', array( $this, 'add_feature_support_to_product_types' ) );
		add_filter( 'ninja_shop_get_product_feature_page-template', array( $this, 'get_feature' ), 9, 2 );
		add_filter( 'ninja_shop_product_has_feature_page-template', array( $this, 'product_has_feature') , 9, 2 );
		add_filter( 'ninja_shop_product_supports_feature_page-template', array( $this, 'product_supports_feature') , 9, 2 );
		add_action( 'ninja_shop_update_product_feature_page-template', array( $this, 'save_feature' ), 9, 3 );
	}
	...
	*/
	function print_metabox( $post ) {
		...
		<p><?php _e( 'This option will allow you to override the default WordPress template used for this product.', 'it-l10n-ithemes-exchange' ); ?> <span class="tip" title="<?php esc_attr_e( __( 'Ninja Shop uses page.php as it\'s default when no custom page templates are available. Page templates for Ninja Shop products need to include a call to the_content() or use Ninja Shop Theme API functions to display correctly.', 'ninja-shop' ) ); ?>">i</span></p>
	...
	*/
	function save_feature_on_product_save() {
		...
		if ( ! empty( $_POST['it-exchange-page-template'] ) )
			it_exchange_update_product_feature( $product_id, 'page-template', sanitize_text_field( $_POST['it-exchange-page-template'] ) );
	}
