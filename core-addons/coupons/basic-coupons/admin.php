...
function it_exchange_basic_coupons_save_coupon() {
	...
	unset(
		$data['code'], $data['amount-number'], $data['amount-type'], $data['start-date'], $data['end-date'],
		$data['limit-quantity'], $data['quantity'], $data['limit-product'], $data['product-id'], $data['limit-frequency'],
		$data['frequency-times'], $data['frequency-length'], $data['frequency-units']
	);
...
function it_exchange_basic_coupons_add_menu_item() {
	if ( ! empty( $_GET['page'] ) && 'it-exchange-add-basic-coupon' == $_GET['page'] ) {
		...
		add_submenu_page( 'it-exchange', __( 'Add Coupon', 'it-l10n-ithemes-exchange' ), __( 'Add Coupon', 'it-l10n-ithemes-exchange' ), 'create_it_coupons', $slug, $func );
	} else if ( ! empty( $_GET['page'] ) && 'it-exchange-edit-basic-coupon' == $_GET['page'] ) {
		$slug = 'it-exchange-edit-basic-coupon';
		$func = 'it_exchange_basic_coupons_print_add_edit_coupon_screen';
		add_submenu_page( 'it-exchange', __( 'Edit Coupon', 'it-l10n-ithemes-exchange' ), __( 'Edit Coupon', 'it-l10n-ithemes-exchange' ), 'edit_it_coupons', $slug, $func );
	}
	$url = add_query_arg( array( 'post_type' => 'it_exchange_coupon' ), 'edit.php' );
	add_submenu_page( 'it-exchange', __( 'Coupons', 'it-l10n-ithemes-exchange' ), __( 'Coupons', 'it-l10n-ithemes-exchange' ), 'edit_it_coupons', $url );
}
...
function it_exchange_basic_coupons_redirect_core_add_edit_screens() {
	...
	$post_id   = empty( $_GET['post'] ) ? false : sanitize_text_field( $_GET['post'] );
	...
	if ( 'post.php' == $pagenow ) {
		if ( $post_id && $coupon ) {
			...
			wp_safe_redirect( add_query_arg( array(
				'page' => 'it-exchange-edit-basic-coupon',
				'post' => $post_id
...
function it_exchange_basic_coupons_print_add_edit_coupon_screen() {
	...
	$form_action = $post_id ? add_query_arg( array(
		'page' => 'it-exchange-edit-basic-coupon',
		'post' => $post_id
	), get_admin_url() . 'admin.php' ) : add_query_arg( array( 'page' => 'it-exchange-add-basic-coupon' ), get_admin_url() . 'admin.php' );
    ...
    <div class="wrap">
        ...
        <div class="it-exchange-add-basic-coupon">
            <div class="fields">
                ...
                <div class="field">
                    <label for="name"><?php _e( 'Name', 'it-l10n-ithemes-exchange' ); ?> <span class="tip"
                                                                                               title="<?php _e( 'What do you want to call this coupon? This is just for your reference.', 'it-l10n-ithemes-exchange' ); ?>">i</span></label>
					<?php $form->add_text_box( 'name' ); ?>
                </div>
                <div class="field coupon-code">
                    <label for="code"><?php _e( 'Code', 'it-l10n-ithemes-exchange' ); ?> <span class="tip"
                                                                                               title="<?php _e( 'Try something cool like EXCHANGERULEZ5000! Or click the dice to generate a random code.', 'it-l10n-ithemes-exchange' ); ?>">i</span></label>
					<?php $form->add_text_box( 'code', array( 'class' => 'emptycode' ) ); ?>
                    <a href class="dice" title="Generate a random code."><img
                                src="<?php echo esc_attr( ITUtility::get_url_from_file( dirname( __FILE__ ) ) ); ?>/images/dice-t.png"/></a>
                </div>
                ...
                <div class="field amount">
					...
					?>
					<?php $form->add_drop_down( 'amount-type', array(
						'%'      => __( '% Percent', 'it-l10n-ithemes-exchange' ),
                        ...
                        <div class="inner">
                            <div class="inside">
                                <div class="field date"
                                     data-alert="<?php _e( 'Please select an end date that is after the start date.', 'it-l10n-ithemes-exchange' ); ?>">
                                    <div class="start-date">
                                        <label for="start-date"><?php _e( 'Start Date', 'it-l10n-ithemes-exchange' ); ?></label>
										<?php $form->add_text_box( 'start-date', array(
											'class'       => 'datepicker',
                                    ...
                                    <div class="end-date">
										...
										<?php $form->add_text_box( 'end-date', array(
											'class'       => 'datepicker',
                                ...
                                <div class="field discount-method">
									...
									<?php $form->add_drop_down( 'apply-discount', array(
										'cart'    => __( 'Entire Cart', 'it-l10n-ithemes-exchange' ),
										'product' => __( 'Per-product', 'it-l10n-ithemes-exchange' )
									) ); ?>
                                </div>
                            ...
                            <div class="inside">
                                <div class="field limit-quantity">
                                    ...
                                    <label for="limit-quantity">
                                        ...
                                        <span class="tip"
                                              title="<?php esc_attr_e( __( 'Check to limit the number of times this coupon can be used. This limit is applied globally.', 'it-l10n-ithemes-exchange' ) ); ?>">i</span>
                                    </label>
                                </div>
                                ...
                                <div class="field quantity">
                                    ...
                                    <span class="tip"
                                          title="<?php _e( 'How many times can this coupon be used before it is disabled?', 'it-l10n-ithemes-exchange' ); ?>">i</span>
                            ...
                            <div class="inside">
                                <div class="field limit-customer">
                                    ...
                                    <label for="limit-customer">
                                        ...
                                        <span class="tip"
                                              title="<?php esc_attr_e( __( 'Check to limit the coupon discount to a specific customer.', 'it-l10n-ithemes-exchange' ) ); ?>">i</span>
                                    </label>
                                </div>
                                ...
                                <div class="field limit-frequency">
                                    ...
                                    <label for="limit-frequency">
                                        ...
                                        <span class="tip"
                                              title="<?php esc_attr_e( __( 'Check to limit the number of times each customer can use the coupon during a specified time frame', 'it-l10n-ithemes-exchange' ) ); ?>">i</span>
                                    </label>
                                </div>
                                ...
                                <div class="field frequency-limitations">
									...
									$frequency_units  = array(
										'day'  => __( 'Day(s)', 'it-l10n-ithemes-exchange' ),
										'week' => __( 'Week(s)', 'it-l10n-ithemes-exchange' ),
										'year' => __( 'Year(s)', 'it-l10n-ithemes-exchange' )
									);
                            ...
                            <div class="inside">
                                <div class="field limit-product">
                                    ...
                                    <label for="limit-product">
                                        ...
                                        <span class="tip"
                                              title="<?php esc_attr_e( __( 'Check to limit the discount to a specific product\'s price <em>instead</em> of the cart total.', 'it-l10n-ithemes-exchange' ) ); ?>">i</span>
                                    </label>
                                </div>
								...
								<?php if ( taxonomy_exists( 'it_exchange_category' ) && ( $terms = get_terms( 'it_exchange_category' ) ) ): ?>
                                    ...
                                    <div class="field product-category">
										...
										?>
										<?php $form->add_drop_down( 'product-category[]', array(
                                ...
                                <div class="field product-id">
									...
									?>
									<?php $form->add_drop_down( 'product-id[]', array(
                                ...
                                <div class="field excluded-products">
									...
									?>
									<?php $form->add_drop_down( 'excluded-products[]', array(
										'value'    => $product_options,
										'multiple' => true
									) ); ?>
                                </div>
                ...
                <div class="field">
					<?php $form->add_submit( 'cancel', array(
						'class' => 'button-large button',
						'value' => __( 'Cancel', 'it-l10n-ithemes-exchange' )
					) ); ?>
					<?php $form->add_submit( 'submit', array(
						'class' => 'button-large button-primary button',
						'value' => __( 'Save', 'it-l10n-ithemes-exchange' )
					) ); ?>
                </div>
            </div>
        </div>
...
function it_exchange_basic_coupons_add_availability_filter( $post_type ) {
    ...
    <label for="start-date"
           class="screen-reader-text"><?php _e( 'Availability Start Date', 'it-l10n-ithemes-exchange' ); ?></label>
    <label for="end-date"
           class="screen-reader-text"><?php _e( 'Availability End Date', 'it-l10n-ithemes-exchange' ); ?></label>
