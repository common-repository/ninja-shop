<?php
/**
 * This is the default template for the Easy EU Value Added Taxes
 * element in the totals loop of the content-checkout
 * template part. It was added by Easy EU Value Added Taxes add-on.
 *
 * 
 * @version 1.1.0
 * @package exchange-addon-easy-eu-value-added-taxes
 *
 * WARNING: Do not edit this file directly. To use
 * this template in a theme, copy over this file
 * to the exchange/content-checkout/elements/
 * directory located in your theme.
*/
?>

<?php do_action( 'ninja_shop_content_cart_before_easy_eu_valued_added_taxes_element' ); ?>
<div class="it-exchange-table-column it-exchange-cart-totals-title">
	<div class="it-exchange-table-column-inner">
		<?php _e( 'Tax', 'LION' ); ?>
	</div>
</div>
<div class="it-exchange-table-column it-exchange-cart-totals-amount">
	<div class="it-exchange-table-column-inner">
		<?php it_exchange( 'eu-value-added-taxes', 'vat-number' ); ?>
	</div>
</div>
<?php do_action( 'ninja_shop_content_cart_after_easy_eu_valued_added_taxes_element' ); ?>
