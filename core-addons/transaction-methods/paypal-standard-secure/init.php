...
/**
 * Gets Ninja Shop's Transaction ID from PayPal Standard Secure's Transaction ID
 *
 * @since 1.10.0
 *
	...
	*/
	...
	function get_paypal_standard_secure_payment_form_table( $form, $settings = array() ) {
		...
		<div class="it-exchange-addon-settings it-exchange-paypal-addon-settings">
			<p>
				<?php _e( 'Although this PayPal version for Ninja Shop takes more effort and time, it is well worth it for the security options for your store. To get PayPal set up for use with Exchange, you\'ll need to add the following information from your PayPal account.', 'it-l10n-ithemes-exchange' ); ?><br /><br />
	...
	function paypal_standard_secure_save_wizard_settings() {
		...
		foreach( (array) $default_wizard_paypal_standard_secure_settings as $var ) {
			...
			if ( isset( $_REQUEST['it_exchange_settings-paypal-standard-secure-' . $var] ) ) {
				$paypal_standard_secure_settings[$var] = sanitize_text_field( $_REQUEST['it_exchange_settings-paypal-standard-secure-' . $var] );
			}
