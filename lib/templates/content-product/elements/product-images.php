<?php
/**
 * The default template part for the product
 * images in one of the the content-product
 * template part's loops.
 *
 * 
 * @version 1.1.0
 * @package IT_Exchange
 *
 * WARNING: Do not edit this file directly. To use
 * this template in a theme, simply copy this file's
 * content to the exchange/content-product/elements
 * directory located in your theme.
*/
?>

<?php if ( it_exchange( 'product', 'has-images' ) ) : ?>
	<?php do_action( 'ninja_shop_content_product_before_product_images_element' ); ?>
	<div class="ninja-shop-column ninja-shop-product-images">
		<div class="ninja-shop-column-inner">
			<?php it_exchange( 'product', 'gallery' ); ?>
		</div>
	</div>
	<?php do_action( 'ninja_shop_content_product_after_product_images_element' ); ?>
<?php endif; ?>
